...
* **Example:** A single Repair Order line can have a "Warranty" pay type for the transmission part, but a "Customer Pay" type for the labor hours.
...
2. **Line Item Level:** The `BillOfMaterialReference` for the "Water Pump" part carries its own `capturedFinancialSplit`. In *this* specific split, `payType = WARRANTY`.
3. **The Result:** The DMS can generate two distinct accounting entries (and potentially two separate invoice statements) from a single `LaborOperation`.
...
3. **Cost vs. Price:** Every `Price` object explicitly separates `totalCostAmount` (Dealer Cost) from `totalAmount` (Selling Price).
