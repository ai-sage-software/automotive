...
This contains the OpenAPI specification for the **Automotive Retail Systems API**, which provides an interface for managing automotive retail entities such as **EffectivePeriod**, **Identifier**, **Money**, **PayrollComponent**, **PayrollStub**, **TaxReportingContext**, **TimeSlot**.
...
> `https://[Your-API-Host]/payrollstubs`
...
## Bulk Create PayrollStub (`POST /payrollstubs/batch`)

The `/payrollstubs/batch` endpoint allows you to create multiple PayrollStub entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of PayrollStub and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PayrollStubCreate` schema.
...
Contains the full objects of the PayrollStub that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A PayrollStub with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /payrollstubs/health`)

The `/payrollstubs/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the PayrollStub aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## PayrollStub Commands and State Management

The payrollstubs API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /payrollstubs/{PayrollStubKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a PayrollStub (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /payrollstubs/{PayrollStubKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /payrollstubs/capabilities`)

Before sending a command, you can "discover" what actions are valid for a PayrollStub based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the PayrollStub must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPayrollStub` | `CREATED`, `ACTIVE` | `DELETED` | Voids the PayrollStub . |
| `ActivatePayrollStub` | `PENDING` | `ACTIVE` | Moves a PayrollStub into the operational pool. |
