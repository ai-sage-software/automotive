# 🚗 STAR Security Contract for PayrollCycle API
This README provides an overview of the security architecture and implementation requirements for the **Automotive Retail Systems PayrollCycle API**.
All developers and integrators must adhere to these standards to ensure the integrity and confidentiality of automotive retail PayrollCycle data.
...
Most endpoints (e.g., `/payroll-cycles`, `/payroll-cycles/batch`) require authentication. A request is considered authorized if it satisfies **either** the OAuth2 requirements **or** provides a valid Member API Key:
...
* `GET /payroll-cycles/health`: Used by load balancers and uptime monitors.
...
When using the `/payroll-cycles/exports` endpoint, the system sends a POST notification to your `webhook_url`. To secure this communication:
