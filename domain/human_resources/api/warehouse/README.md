...
The API is structured around the domain **human_resources** and **Warehouse** resource as the primary entity, with several resources scoped under it via various path parameters.
...
> `https://[Your-API-Host]/warehouses`
...
## Bulk Create Warehouse (`POST /warehouses/batch`)

The `/warehouses/batch` endpoint allows you to create multiple Warehouse entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of Warehouse and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `WarehouseCreate` schema.
...
Contains the full objects of the Warehouse that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A Warehouse with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /warehouses/health`)

The `/warehouses/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the Warehouse aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## Warehouse Commands and State Management

The warehouses API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /warehouses/{WarehouseKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a Warehouse (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /warehouses/{WarehouseKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /warehouses/capabilities`)

Before sending a command, you can "discover" what actions are valid for a Warehouse based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the Warehouse must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelWarehouse` | `CREATED`, `ACTIVE` | `DELETED` | Voids the Warehouse . |
| `ActivateWarehouse` | `PENDING` | `ACTIVE` | Moves a Warehouse into the operational pool. |
