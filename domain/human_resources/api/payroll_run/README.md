...
This contains the OpenAPI specification for the **Automotive Retail Systems API**, which provides an interface for managing automotive retail entities such as **Address**, **AddressLocale**, **Authorization**, **CommunicationChannel**, **ContactMethod**, **EffectivePeriod**, **Identifier**, **PayrollPayment**, **PayrollRun**, **Position**, **ProductSubject**, **TimeSlot**.
...
> `https://[Your-API-Host]/payrollruns`
...
## Bulk Create PayrollRun (`POST /payrollruns/batch`)

The `/payrollruns/batch` endpoint allows you to create multiple PayrollRun entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of PayrollRun and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PayrollRunCreate` schema.
...
Contains the full objects of the PayrollRun that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A PayrollRun with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /payrollruns/health`)

The `/payrollruns/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the PayrollRun aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## PayrollRun Commands and State Management

The payrollruns API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /payrollruns/{PayrollRunKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a PayrollRun (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /payrollruns/{PayrollRunKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /payrollruns/capabilities`)

Before sending a command, you can "discover" what actions are valid for a PayrollRun based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the PayrollRun must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPayrollRun` | `CREATED`, `ACTIVE` | `DELETED` | Voids the PayrollRun . |
| `ActivatePayrollRun` | `PENDING` | `ACTIVE` | Moves a PayrollRun into the operational pool. |
