...
| :--- | :--- | :--- |
    ...
    | **Money** | /budgets/{budgetKey}/money | Manages Money belonging to Budgets |
...
### /budgets/{budgetKey}/money
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
        <span class="api-path">/budgets/{budgetKey}/money</span> <br/>
        <span class="api-summary">Retrieve a list of Money entities scoped by budgetKey. getMoneyByBudgetKey</span>
    </span>
</div>

### /budgets/{budgetKey}/money/{moneyKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
        <span class="api-path">/budgets/{budgetKey}/money/{moneyKey}</span> <br/>
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
    ...
    | **money** | /budgets/{budgetKey}/money | listMoneyByBudgetKey |  | getMoneyByBudgetKey | updateMoneyByBudgetKey | deleteMoneyByBudgetKey |
...
## Bulk Create Budget (`POST /budgets/batch`)

The `/budgets/batch` endpoint allows you to create multiple Budget entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of Budget and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `BudgetCreate` schema.
...
Contains the full objects of the Budget that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A Budget with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /budgets/health`)

The `/budgets/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the Budget aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## Budget Commands and State Management

The budgets API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /budgets/{BudgetKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a Budget (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /budgets/{BudgetKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /budgets/capabilities`)

Before sending a command, you can "discover" what actions are valid for a Budget based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the Budget must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelBudget` | `CREATED`, `ACTIVE` | `DELETED` | Voids the Budget . |
| `ActivateBudget` | `PENDING` | `ACTIVE` | Moves a Budget into the operational pool. |
