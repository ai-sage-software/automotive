...
| :--- | :--- | :--- |
    ...
    | **Money** | /journal-entry-transactions/{journalEntryTransactionKey}/money | Manages Money belonging to JournalEntryTransactions |
...
> `https://[Your-API-Host]/journalentrytransactions`
...
### /journal-entry-transactions/{journalEntryTransactionKey}/money
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
        <span class="api-path">/journal-entry-transactions/{journalEntryTransactionKey}/money</span> <br/>
...
### /journal-entry-transactions/{journalEntryTransactionKey}/money/{moneyKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
        <span class="api-path">/journal-entry-transactions/{journalEntryTransactionKey}/money/{moneyKey}</span> <br/>
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
    ...
    | **money** | /journal-entry-transactions/{journalEntryTransactionKey}/money | listMoneyByJournalEntryTransactionKey |  | getMoneyByJournalEntryTransactionKey | updateMoneyByJournalEntryTransactionKey | deleteMoneyByJournalEntryTransactionKey |
...
## Bulk Create JournalEntryTransaction (`POST /journalentrytransactions/batch`)

The `/journalentrytransactions/batch` endpoint allows you to create multiple JournalEntryTransaction entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of JournalEntryTransaction and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `JournalEntryTransactionCreate` schema.
...
Contains the full objects of the JournalEntryTransaction that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A JournalEntryTransaction with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /journalentrytransactions/health`)

The `/journalentrytransactions/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the JournalEntryTransaction aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## JournalEntryTransaction Commands and State Management

The journalentrytransactions API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /journalentrytransactions/{JournalEntryTransactionKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a JournalEntryTransaction (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /journalentrytransactions/{JournalEntryTransactionKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /journalentrytransactions/capabilities`)

Before sending a command, you can "discover" what actions are valid for a JournalEntryTransaction based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the JournalEntryTransaction must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelJournalEntryTransaction` | `CREATED`, `ACTIVE` | `DELETED` | Voids the JournalEntryTransaction . |
| `ActivateJournalEntryTransaction` | `PENDING` | `ACTIVE` | Moves a JournalEntryTransaction into the operational pool. |
