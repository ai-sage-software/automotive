...
The API is structured around the domain **finance** and **AccountingPeriod** resource as the primary entity, with several resources scoped under it via various path parameters.
...
> `https://[Your-API-Host]/accountingperiods`
...
The following resources follow a consistent pattern under AccountingPeriodroot with key {AccountingPeriodKey} ... Support listing, creation, retrieval, replacement, deletion, and partial updates.
...
## Bulk Create AccountingPeriod (`POST /accountingperiods/batch`)

The `/accountingperiods/batch` endpoint allows you to create multiple AccountingPeriod entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of AccountingPeriod and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `AccountingPeriodCreate` schema.
...
Contains the full objects of the AccountingPeriod that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A AccountingPeriod with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /accountingperiods/health`)

The `/accountingperiods/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the AccountingPeriod aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## AccountingPeriod Commands and State Management

The accountingperiods API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /accountingperiods/{AccountingPeriodKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a AccountingPeriod (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /accountingperiods/{AccountingPeriodKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /accountingperiods/capabilities`)

Before sending a command, you can "discover" what actions are valid for a AccountingPeriod based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the AccountingPeriod must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelAccountingPeriod` | `CREATED`, `ACTIVE` | `DELETED` | Voids the AccountingPeriod . |
| `ActivateAccountingPeriod` | `PENDING` | `ACTIVE` | Moves a AccountingPeriod into the operational pool. |
