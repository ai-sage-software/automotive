...
The API is structured around the domain **finance** and **ControlAccount** resource as the primary entity, with several resources scoped under it via various path parameters.

| Resource | Base Path | Description |
| :--- | :--- | :--- |
    | **ControlAccount** | /control-accounts | Manages ControlAccounts |
    | **Identifier** | /control-accounts/{controlAccountKey}/identifiers | Manages Identifiers belonging to ControlAccounts |
...
> `https://[Your-API-Host]/controlaccounts`
...
### /control-accounts/{controlAccountKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /control-accounts/{controlAccountKey}/identifiers
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /control-accounts/{controlAccountKey}/identifiers/{identifierKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
The following resources follow a consistent pattern under ControlAccountroot with key {ControlAccountKey} ... Support listing, creation, retrieval, replacement, deletion, and partial updates.
...
## Bulk Create ControlAccount (`POST /controlaccounts/batch`)

The `/controlaccounts/batch` endpoint allows you to create multiple ControlAccount entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of ControlAccount and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `ControlAccountCreate` schema.
...
Contains the full objects of the ControlAccount that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A ControlAccount with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /controlaccounts/health`)

The `/controlaccounts/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the ControlAccount aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## ControlAccount Commands and State Management

The controlaccounts API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /controlaccounts/{ControlAccountKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a ControlAccount (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /controlaccounts/{ControlAccountKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /controlaccounts/capabilities`)

Before sending a command, you can "discover" what actions are valid for a ControlAccount based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the ControlAccount must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelControlAccount` | `CREATED`, `ACTIVE` | `DELETED` | Voids the ControlAccount . |
| `ActivateControlAccount` | `PENDING` | `ACTIVE` | Moves a ControlAccount into the operational pool. |
