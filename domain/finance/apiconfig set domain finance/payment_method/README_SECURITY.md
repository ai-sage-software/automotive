# 🚗 STAR Security Contract for PaymentMethod API
This README provides an overview of the security architecture and implementation requirements for the **Automotive Retail Systems PaymentMethod API**.
All developers and integrators must adhere to these standards to ensure the integrity and confidentiality of automotive retail PaymentMethod data.
...
Most endpoints (e.g., `/payment-methods`, `/payment-methods/batch`) require authentication. A request is considered authorized if it satisfies **either** the OAuth2 requirements **or** provides a valid Member API Key:
...
* `GET /payment-methods/health`: Used by load balancers and uptime monitors.
...
When using the `/payment-methods/exports` endpoint, the system sends a POST notification to your `webhook_url`. To secure this communication:
