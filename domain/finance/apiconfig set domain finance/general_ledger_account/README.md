...
> `https://[Your-API-Host]/generalledgeraccounts`
...
💠 **GeneralLedgerAccountTypes** : types of general ledger accounts.<br/>
...
## Bulk Create GeneralLedgerAccount (`POST /generalledgeraccounts/batch`)

The `/generalledgeraccounts/batch` endpoint allows you to create multiple GeneralLedgerAccount entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of GeneralLedgerAccount and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `GeneralLedgerAccountCreate` schema.
...
Contains the full objects of the GeneralLedgerAccount that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A GeneralLedgerAccount with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /generalledgeraccounts/health`)

The `/generalledgeraccounts/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the GeneralLedgerAccount aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## GeneralLedgerAccount Commands and State Management

The generalledgeraccounts API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /generalledgeraccounts/{GeneralLedgerAccountKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a GeneralLedgerAccount (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /generalledgeraccounts/{GeneralLedgerAccountKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /generalledgeraccounts/capabilities`)

Before sending a command, you can "discover" what actions are valid for a GeneralLedgerAccount based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the GeneralLedgerAccount must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelGeneralLedgerAccount` | `CREATED`, `ACTIVE` | `DELETED` | Voids the GeneralLedgerAccount . |
| `ActivateGeneralLedgerAccount` | `PENDING` | `ACTIVE` | Moves a GeneralLedgerAccount into the operational pool. |
