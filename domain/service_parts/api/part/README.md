...
This contains the OpenAPI specification for the **Automotive Retail Systems API**, which provides an interface for managing automotive retail entities such as **ControlAccountReference**, **EffectivePeriod**, **Identifier**, **Part**, **PartEvent**, **PartIdentifier**, **PartLifecycle**, **PartLocation**, **PartName**, **Price**, **SuperSession**, **TimeSlot**.
...
## Bulk Create Part (`POST /parts/batch`)

The `/parts/batch` endpoint allows you to create multiple Part entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of Part and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PartCreate` schema.
...
Contains the full objects of the Part that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A Part with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /parts/health`)

The `/parts/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the Part aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## Part Commands and State Management

The parts API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /parts/{PartKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a Part (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /parts/{PartKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /parts/capabilities`)

Before sending a command, you can "discover" what actions are valid for a Part based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the Part must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPart` | `CREATED`, `ACTIVE` | `DELETED` | Voids the Part . |
| `ActivatePart` | `PENDING` | `ACTIVE` | Moves a Part into the operational pool. |
