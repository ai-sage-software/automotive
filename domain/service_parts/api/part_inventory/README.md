...
This contains the OpenAPI specification for the **Automotive Retail Systems API**, which provides an interface for managing automotive retail entities such as **ControlAccountReference**, **EffectivePeriod**, **Identifier**, **PartInventory**, **PartInventoryEvent**, **PartInventoryMetric**, **PartReference**, **Price**, **TimeSlot**.
...
> `https://[Your-API-Host]/part-inventories`
...
## Bulk Create PartInventory (`POST /part-inventories/batch`)

The `/part-inventories/batch` endpoint allows you to create multiple PartInventory entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of PartInventory and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PartInventoryCreate` schema.
...
Contains the full objects of the PartInventory that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A PartInventory with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /part-inventories/health`)

The `/part-inventories/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the PartInventory aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## PartInventory Commands and State Management

The PartInventories API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /part-inventories/{PartInventoryKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a PartInventory (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /part-inventories/{PartInventoryKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /part-inventories/capabilities`)

Before sending a command, you can "discover" what actions are valid for a PartInventory based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the PartInventory must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPartInventory` | `CREATED`, `ACTIVE` | `DELETED` | Voids the PartInventory . |
| `ActivatePartInventory` | `PENDING` | `ACTIVE` | Moves a PartInventory into the operational pool. |
