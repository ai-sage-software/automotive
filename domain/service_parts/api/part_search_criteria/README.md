...
This contains the OpenAPI specification for the **Automotive Retail Systems API**, which provides an interface for managing automotive retail entities such as **AdvanceSearchValue**, **AdvancedSearchCriteria**, **EffectivePeriod**, **Identifier**, **PartIdentifier**, **PartLifecycle**, **PartLogisticsCriteria**, **PartName**, **PartPriceCriteria**, **PartSearchCriteria**, **PartSpecification**, **SuperSession**, **TimeSlot**, **UnitOfMeasure**, **WarehouseReference**.
...
> `https://[Your-API-Host]/partsearchcriterias`
...
## Bulk Create PartSearchCriteria (`POST /partsearchcriterias/batch`)

The `/partsearchcriterias/batch` endpoint allows you to create multiple PartSearchCriteria entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of PartSearchCriteria and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PartSearchCriteriaCreate` schema.
...
Contains the full objects of the PartSearchCriteria that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A PartSearchCriteria with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /partsearchcriterias/health`)

The `/partsearchcriterias/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the PartSearchCriteria aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## PartSearchCriteria Commands and State Management

The partsearchcriterias API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /partsearchcriterias/{PartSearchCriteriaKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a PartSearchCriteria (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /partsearchcriterias/{PartSearchCriteriaKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /partsearchcriterias/capabilities`)

Before sending a command, you can "discover" what actions are valid for a PartSearchCriteria based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the PartSearchCriteria must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPartSearchCriteria` | `CREATED`, `ACTIVE` | `DELETED` | Voids the PartSearchCriteria . |
| `ActivatePartSearchCriteria` | `PENDING` | `ACTIVE` | Moves a PartSearchCriteria into the operational pool. |
