...
The API is structured around the domain **service_parts** and **PartSearch** resource as the primary entity, with several resources scoped under it via various path parameters.
...
| :--- | :--- | :--- |
    ...
    | **PartIdentifier** | /part-searches/{partSearchKey}/part-identifiers | Manages PartIdentifiers belonging to PartSearches |
...
> `https://[Your-API-Host]/part-searches`
...
### /part-searches/{partSearchKey}/part-search-results
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/part-search-results/{partSearchResultKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/identifiers
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/part-names
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/part-names/{partNameKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/part-identifiers
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
### /part-searches/{partSearchKey}/part-identifiers/{partIdentifierKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
The following resources follow a consistent pattern under PartSearchroot with key {PartSearchKey} ... Support listing, creation, retrieval, replacement, deletion, and partial updates.
...
## Bulk Create PartSearch (`POST /part-searches/batch`)

The `/part-searches/batch` endpoint allows you to create multiple PartSearch entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of PartSearch and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `PartSearchCreate` schema.
...
Contains the full objects of the PartSearch that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A PartSearch with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /part-searches/health`)

The `/part-searches/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the PartSearch aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## PartSearch Commands and State Management

The PartSearches API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /part-searches/{PartSearchKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a PartSearch (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /part-searches/{PartSearchKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /part-searches/capabilities`)

Before sending a command, you can "discover" what actions are valid for a PartSearch based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the PartSearch must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelPartSearch` | `CREATED`, `ACTIVE` | `DELETED` | Voids the PartSearch . |
| `ActivatePartSearch` | `PENDING` | `ACTIVE` | Moves a PartSearch into the operational pool. |
