# 🚗 STAR Security Contract for PartSearch API
This README provides an overview of the security architecture and implementation requirements for the **Automotive Retail Systems PartSearch API**.
All developers and integrators must adhere to these standards to ensure the integrity and confidentiality of automotive retail PartSearch data.
...
Most endpoints (e.g., `/part-searches`, `/part-searches/batch`) require authentication. A request is considered authorized if it satisfies **either** the OAuth2 requirements **or** provides a valid Member API Key:
...
* `GET /part-searches/health`: Used by load balancers and uptime monitors.
...
When using the `/part-searches/exports` endpoint, the system sends a POST notification to your `webhook_url`. To secure this communication:
