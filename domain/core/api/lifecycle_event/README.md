...
The API is structured around the domain **core** and **LifecycleEvent** resource as the primary entity, with several resources scoped under it via various path parameters.

| Resource | Base Path | Description |
| :--- | :--- | :--- |
    | **LifecycleEvent** | /lifecycle-events | Manages LifecycleEvents |
...
> `https://[Your-API-Host]/lifecycleevents`
...
### /lifecycle-events/{lifecycleEventKey}
<div class="api-endpoint-row">
<span class="api-method-button method-get">GET</span>
    <span class="api-path-summary">
...
The following resources follow a consistent pattern under LifecycleEventroot with key {LifecycleEventKey} ... Support listing, creation, retrieval, replacement, deletion, and partial updates.
...
## Bulk Create LifecycleEvent (`POST /lifecycleevents/batch`)

The `/lifecycleevents/batch` endpoint allows you to create multiple LifecycleEvent entities in a single HTTP request. This is significantly more efficient than making individual calls when dealing with large datasets, as it reduces network overhead and connection latency.

### Overview

This endpoint processes an array of LifecycleEvent and provides a **partial success** response. This means that if some items in your list are invalid, the valid items will still be created, and the API will return a detailed report of which specific items failed.
...
* **Schema:** Each object in the `items` array should follow the `LifecycleEventCreate` schema.
...
Contains the full objects of the LifecycleEvent that were successfully created, including server-generated fields like `id` or `createdAt`.
...
"error": {
"code": "DUPLICATE_SKU",
"message": "A LifecycleEvent with SKU W-002 already exists."
}
}
]
}
...
## Health Check (`GET /lifecycleevents/health`)

The `/lifecycleevents/health` endpoint is a public diagnostic tool used to verify the current availability and operational state of the service. It is designed to be used by load balancers, monitoring tools, and automated deployment pipelines.

---

### Overview

This endpoint provides a quick "heartbeat" of the LifecycleEvent aggregated root. Because it is a public endpoint, it typically does not require authentication, making it ideal for external uptime monitors (like Pingdom or UptimeRobot) to ensure the service is responsive.
...
## LifecycleEvent Commands and State Management

The lifecycleevents API can handle complex business workflows. Rather than simple attribute updates, state changes are driven by explicit "Commands" that represent business intents.

---

### 1. Executing Commands (`POST /lifecycleevents/{LifecycleEventKey}/commands`)

This endpoint is the single entry point for all intents that change the state of a LifecycleEvent (e.g., rescheduling, cancelling, or deleting).
...
### 2. Command Polling (`GET /lifecycleevents/{LifecycleEventKey}/commands/{commandId}`)
...
### 3. Capability Discovery (`GET /lifecycleevents/capabilities`)

Before sending a command, you can "discover" what actions are valid for a LifecycleEvent based on its current state. This endpoint describes the **State Machine** governing the resource.

**Key Fields:**

* **`from_states`**: The states the LifecycleEvent must be in to accept this command.
...
| --- | --- | --- | --- |
| `CancelLifecycleEvent` | `CREATED`, `ACTIVE` | `DELETED` | Voids the LifecycleEvent . |
| `ActivateLifecycleEvent` | `PENDING` | `ACTIVE` | Moves a LifecycleEvent into the operational pool. |
